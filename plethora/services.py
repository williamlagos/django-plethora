...
class ContentService:
    ...
    def view_spreaded(self, request):
        ...
        spreadables.append(
            globals()[typ].objects.filter(id=sprdd[0].spread)[0])
        for s in sprdd:
            spreadables.append(Spreadable.objects.filter(id=s.spreaded)[0])
    ...
    def deadline(self):
        ...
        for play in playables:
            if not play.token and not play.visual:
                play.delete()

    def relations(self, feed):
        excludes = []
        rels = Spreaded.objects.filter(user=self.user)
    ...
    def duplicates(self, exclude, feed):
        for o in self.objects:
            ...
            if 'Spreadable' in o:
                e = list(filter(sp, exclude))
            elif 'Playable' in o:
                e = list(filter(pl, exclude))
            elif 'Image' in o:
                e = list(filter(im, exclude))
